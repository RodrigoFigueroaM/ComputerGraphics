        ...
        for x in range(i - degree, i + 1, 1):
            for y in range(0, x + 1, 1):
                if y == 0:
                    ...
                    dictionary[str(x) + ',' + str(y) + ',' + str(u)] = dictionary[str(x) + ',' + str(y - 1) + ',' + str(
                        u)] * top + bottomVal * bottom

        return dictionary[str(i) + ',' + str(degree) + ',' + str(u)]

    def __coxDeBoorHomogenousCoordinatesWithDictionary(self, degree=None, knots=None, u=None, i=None,
                                                       homogenousCoordinates=None):
        '''original CoxDeBoor algorithm computed with dictionaries'''
        top = 1
        bottom = 1
